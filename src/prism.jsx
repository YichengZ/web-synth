...
const { useState, useEffect, useRef, useCallback } = React;
    ...
    const Icons = {
        ...
        Home: (props) => <IconBase {...props}><path d="m3 9 9-7 9 7"/><path d="M9 22V12h6v10"/><path d="M21 9v13H3V9"/></IconBase>,
    ...
    const { Play, Pause, Home, Dice5, Lock, Unlock, Sparkles, Sun, Hexagon, Aperture, Activity, Settings2, Moon, Stars, Wind, Zap, Waves, Snowflake, Radio, Bell, Droplets, CircleDot, Layers, Beaker, Music, Smile, Star, Mic, Square } = Icons;
    ...
    const CrystalSynth = () => {
      ...
      const [activePreset, setActivePreset] = usePersistentState('web-synth.prism.preset', 'POP');
      const [params, setParams] = usePersistentState('web-synth.prism.params', {...PRESETS.POP, bpm: 120, arp: 'OFF', arpRate: 0.5, arpOctave: 1, arpGate: 0.8});
      const [masterVolume, setMasterVolume] = usePersistentState('web-synth.prism.volume', 0.8);

      const paramsRef = useRef(params);
      useEffect(() => { paramsRef.current = params; }, [params]);

      const [lockedParams, setLockedParams] = usePersistentState('web-synth.prism.locks', {});
        ...
        if (!audioCtxRef.current) {
          ...
          recorderRef.current = createWavRecorder(ctx, analyser, {
            onLimit: async () => {
              ...
              await recorderRef.current?.stop(`PRISM_REC_LIMIT_${ctx.sampleRate}hz_24bit_${timestamp}.wav`);
            }
          });
      ...
      }, [masterVolume, updateShimmerBuffer]);
      ...
      const toggleRecording = useCallback(async () => {
          ...
          if (isRecording) {
              setIsRecording(false);
              const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
              await recorderRef.current.stop(`PRISM_REC_${paramsRef.current.engine}_${audioCtxRef.current.sampleRate}hz_24bit_${timestamp}.wav`);
          } else {
      ...
      // --- VISUALIZER ---
      useEffect(() => {
        if (!analyserRef.current || !canvasRef.current) return undefined;
        ...
        const draw = () => {
          ...
          try {
              ...
              console.warn('PRISM visualizer paused after a drawing error.', error);
          }
      ...
      useEffect(() => {
        ...
        const handleKeyUp = (event) => {
          if (event.code === 'Space' && !isTypingTarget(event.target)) handleMouseUp();
        };
      ...
      }, [handleMouseDown, handleMouseUp, toggleRecording]);
      ...
      return (
        <div className="min-h-screen bg-slate-950 text-cyan-50 font-sans p-0 sm:p-4 lg:p-6 flex flex-col items-stretch lg:items-center justify-center">

          <div className="w-full max-w-7xl min-h-screen lg:min-h-0 bg-slate-900 border border-slate-800 rounded-none sm:rounded-2xl shadow-2xl overflow-hidden flex flex-col lg:flex-row">

            {/* SIDEBAR */}
            <div className="w-full lg:w-64 bg-slate-950 border-b lg:border-b-0 lg:border-r border-slate-800 p-4 flex flex-col shrink-0">
               <div className="flex items-center justify-between gap-2 mb-3 lg:mb-2">
                  ...
                  <a href="index.html" aria-label="Back to AI Synth Lab" className="lg:hidden h-9 px-3 rounded-lg border border-slate-800 text-slate-400 flex items-center gap-2 text-[10px] font-bold tracking-widest">
                    <Home className="w-3.5 h-3.5" /> LAB
                  </a>
               </div>

               <button onClick={() => setIsPresetMenuOpen(open => !open)} aria-expanded={isPresetMenuOpen} className="lg:hidden h-10 px-3 mb-3 rounded-lg border border-slate-800 text-cyan-200 flex items-center justify-between text-xs font-bold tracking-widest">
                 ENGINES <span aria-hidden="true">{isPresetMenuOpen ? '−' : '+'}</span>
               </button>

               {/* --- CREDITS SECTION --- */}
               <div className="hidden lg:block mb-6 px-1 border-b border-slate-800/50 pb-4">
                   <div className="text-xs font-bold text-slate-400 mb-1">DEVELOPER</div>
                   <a href="https://linktr.ee/yichengzhu316" target="_blank" rel="noopener noreferrer" className="block text-sm font-bold text-white hover:text-cyan-300 transition-colors mb-1">
                       Ethan Zhu
                   </a>
                   <div className="text-[10px] text-slate-600 font-mono mb-3">yichengzhu@outlook.com</div>

                   <div className="text-[10px] text-slate-600">
                       Special thanks to <a href="https://linktr.ee/amonova" target="_blank" rel="noopener noreferrer" className="text-slate-500 hover:text-pink-300 transition-colors underline decoration-slate-700">Amo Nova</a>
                   </div>
                   <a href="index.html" aria-label="Back to AI Synth Lab" className="mt-4 h-9 px-3 rounded-lg border border-slate-800 text-slate-400 hover:text-white hover:bg-slate-900 flex items-center justify-center gap-2 text-[10px] font-bold tracking-widest transition-colors">
                       <Home className="w-3.5 h-3.5" /> LAB
                   </a>
               </div>

               <div className={`${isPresetMenuOpen ? 'block' : 'hidden'} lg:block space-y-1 mb-6 lg:flex-1 lg:overflow-y-auto lg:max-h-[500px] lg:pr-2`}>
               ...
               <div className={`${isPresetMenuOpen ? 'grid' : 'hidden'} lg:block grid-cols-1 gap-2 lg:space-y-3 pt-4 border-t border-slate-800`}>
                  ...
                  <button onClick={toggleRecording} className={`hidden lg:flex w-full py-3 border rounded-xl items-center justify-center gap-2 text-xs font-bold transition-all active:scale-95 ${isRecording ? 'bg-red-900/50 border-red-500 text-red-200' : 'border-slate-700 hover:bg-slate-800 text-slate-300'}`}>
                      {isRecording ? <><Square className="w-4 h-4 fill-current animate-pulse" /> STOP {recordingTime}</> : <><Mic className="w-4 h-4" /> REC WAV</>}
                  </button>
               </div>
            </div>

            {/* MAIN PANEL */}
            <main className="flex-1 min-w-0 flex flex-col bg-slate-950 relative">

               <div className="min-h-16 border-b border-slate-800 bg-slate-900/50 flex flex-wrap items-center px-4 sm:px-6 py-3 gap-4 justify-between sticky top-0 z-30 lg:static">
                   <div className="flex items-center gap-4">
                       <button onClick={toggleSequencer} aria-label={isPlaying ? 'Stop sequencer' : 'Start sequencer'} className={`w-10 h-10 rounded-full flex items-center justify-center transition-all ${isPlaying ? 'bg-pink-500 text-white shadow-[0_0_15px_rgba(236,72,153,0.5)]' : 'bg-slate-800 text-slate-400 hover:text-white'}`}>
                   ...
                   <div className="order-3 sm:order-none w-full sm:w-auto sm:flex-1 flex justify-center">
                       {isRecording && (
                           <div className="hidden lg:flex items-center gap-2 bg-red-500/10 px-4 py-1 rounded-full border border-red-500/50 record-pulse">
                               <div className="w-2 h-2 bg-red-500 rounded-full"></div>
                               <span className="text-[10px] font-bold text-red-400 tracking-widest">REC 24-BIT · {recordingTime}</span>
                           </div>
                       )}
                       <button onClick={toggleRecording} className={`lg:hidden h-10 px-4 rounded-lg border text-[10px] font-bold tracking-widest ${isRecording ? 'border-red-500 text-red-200 record-pulse' : 'border-slate-700 text-slate-300'}`}>
                   ...
                   <div className="flex items-center gap-3 sm:gap-6">
                       ...
                       <div className="flex flex-col w-24">
                           ...
                           <input aria-label="Main volume" type="range" min={0} max={1} step={0.01} value={masterVolume} onChange={e => setMasterVolume(parseFloat(e.target.value))} className="w-full h-1 bg-slate-700 accent-white rounded-lg cursor-pointer" />
                       </div>
                       ...
                       <div className="flex flex-col w-32">
                           ...
                           <input aria-label="Sequencer speed" type="range" min={60} max={600} value={params.bpm} onChange={e => updateParam('bpm', e.target.value)} className="w-full h-1 bg-slate-700 accent-pink-500 rounded-lg cursor-pointer" />
                       </div>
                   </div>
               </div>

               {/* VISUALIZER */}
               <div className="h-56 relative cursor-pointer group border-b border-slate-800/50 touch-none"
                    role="button"
                    tabIndex="0"
                    aria-label="Hold to trigger PRISM sound"
               ...
               <div className="p-5 sm:p-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">

                  {/* THEORY & ARP COLUMN */}
                  <div className="space-y-4 col-span-1 sm:border-r border-slate-800/50 sm:pr-6">
                      <Label icon={Music} text="THEORY" />
                      <div className="space-y-1">
                          {/* SCALE */}
                          <div className="flex justify-between items-center mb-1">
                            <label className="text-[10px] font-bold text-slate-400">SCALE</label>
                            <button aria-label={`${lockedParams.scale ? 'Unlock' : 'Lock'} scale`} aria-pressed={lockedParams.scale} onClick={() => toggleLock('scale')} className={`${lockedParams.scale ? 'text-cyan-200' : 'text-slate-600'} hover:text-cyan-100`}><Lock className="w-3 h-3" /></button>
                          </div>
                          <select
                            aria-label="Scale"
                          ...
                          <div className="flex justify-between items-center mb-1">
                            ...
                            <button aria-label={`${lockedParams.arp ? 'Unlock' : 'Lock'} arpeggiator mode`} aria-pressed={lockedParams.arp} onClick={() => toggleLock('arp')} className={`${lockedParams.arp ? 'text-cyan-200' : 'text-slate-600'} hover:text-cyan-100`}><Lock className="w-3 h-3" /></button>
                          </div>
                          <select
                          ...
                          <div className="flex justify-between items-center mb-1">
                            ...
                            <button aria-label={`${lockedParams.arpRate ? 'Unlock' : 'Lock'} arpeggiator rate`} aria-pressed={lockedParams.arpRate} onClick={() => toggleLock('arpRate')} className={`${lockedParams.arpRate ? 'text-cyan-200' : 'text-slate-600'} hover:text-cyan-100`}><Lock className="w-3 h-3" /></button>
                          </div>
                          <select
    ...
    const EngineBtn = ({ id, title, desc, icon: Icon, active, color, onClick }) => (
      <button onClick={onClick} aria-pressed={active} className={`w-full text-left p-3 rounded-xl border transition-all duration-300 flex items-center gap-3 group relative overflow-hidden ${active ? 'bg-slate-900 border-slate-700 shadow-xl shadow-cyan-900/10' : 'border-transparent hover:bg-slate-900/40'}`}>
      ...
      <div className="relative group">
         <div className="flex justify-between items-end mb-2">
            ...
            <div className="flex items-center gap-2">
               ...
               <button onClick={onToggleLock} aria-label={`${locked ? 'Unlock' : 'Lock'} ${label}`} aria-pressed={locked} className={`hover:bg-slate-800 rounded p-0.5 transition-colors ${locked ? 'text-cyan-200' : 'text-slate-700 hover:text-slate-400'}`}>
                 {locked ? <Lock className="w-3 h-3" /> : <Unlock className="w-3 h-3" />}
               </button>
            </div>
         </div>
         <input aria-label={label} type="range" min={min} max={max} step={step} value={value || 0} onChange={(e) => onChange(e.target.value)}
